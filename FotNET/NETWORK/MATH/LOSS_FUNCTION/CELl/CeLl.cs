        ...
        for (var channel = 0; channel < outputTensor.Channels.Count; channel++)
            for (var x = 0; x < outputTensor.Channels[channel].Rows; x++)
                for (var y = 0; y < outputTensor.Channels[channel].Columns; y++)
                    sum += Math.Abs(expectedTensor.Channels[channel].Body[x, y] *
                                    Math.Log(outputTensor.Channels[channel].Body[x, y]) +
                                    (1 - expectedTensor.Channels[channel].Body[x, y]) *
                                    Math.Log(1 - outputTensor.Channels[channel].Body[x, y]));
    ...
    public override Tensor GetErrorTensor(Tensor outputTensor, Tensor expectedTensor) {
        ...
        for (var channel = 0; channel < outputTensor.Channels.Count; channel++) {
            ...
            for (var j = 0; j < outputTensor.Channels[channel].Columns; j++)
                tensor.Channels[^1].Body[i, j] =
                    (outputTensor.Channels[channel].Body[i, j] - expectedTensor.Channels[channel].Body[i, j]) /
                    (outputTensor.Channels[channel].Body[i, j] * (1 - outputTensor.Channels[channel].Body[i, j]));
        }

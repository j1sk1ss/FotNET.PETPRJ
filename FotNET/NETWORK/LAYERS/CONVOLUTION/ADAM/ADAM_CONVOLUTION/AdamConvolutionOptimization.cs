        ...
        if (update && backPropagate) {
            ...
            Parallel.For(0, filters.Length, filter => {
                ...
                for (var channel = 0; channel < filters[filter].Channels.Count; channel++) {
                    ...
                    momentum[filter].Channels[channel] = momentum[filter].Channels[channel] * firstBeta + grad * (1 - firstBeta);
                    velocity[filter].Channels[channel] = velocity[filter].Channels[channel] * secondBeta + grad * grad * (1 - secondBeta);
                    
                    var momentumHat = momentum[filter].Channels[channel] / (1 - Math.Pow(firstBeta, current));
                    var velocityHat = velocity[filter].Channels[channel] / (1 - Math.Pow(secondBeta, current));
